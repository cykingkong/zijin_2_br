<template>
  <div>
    <div class="contract-balance pt-12 px-12 flex gap-2">
      <div class="tab-item font-size-12 px-7 py-5" :class="{ 'bg-#1678FF rounded-10px ': timeActive == item }"
        @click="timeActive = item" v-for="(item, index) in time" :key="index">
        {{ item }}
      </div>
    </div>
...
const initChart = () => {
  ...
  marketkline()
  chart.value.setStyles(theme('dark'))
  ...
  ws.on("message", res => {
    if (res.code == 200) {
      if (res.data && res.msgType != 'broadcast.kline') {
        ...
        // store.commit('setlistData', res.data.tick);
...
const marketkline = () => {
  http.marketkline({
    trading_pairs_id: props.trading_pair_id,
    period: period
  }).then(res => {
    // store.commit('setlistData', res.data[res.data.length - 1]);
