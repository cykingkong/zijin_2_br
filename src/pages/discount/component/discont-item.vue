  ...
  <div>
    <div class="discont-item-content w-full overflow-hidden flex flex-col mb-29px border-bottom"
      ...
      <div class="top-li flex items-center relative">
        <div class="mark absolute top-0 left-0 w-full h-full z-10" @click="handleClickItem"></div>
      ...
      <div class="li pt-20px pb-24 li-x-border flex justify-between items-center px-12 li-border">
        <div class="li-l font-size-18 flex flex-col gap-4px w-full">
          ...
          <div class="font-size-16 text-#64748B">
            {{ _item.discount_rate }}% <span class="up ml-8px" v-if="_item.is_reserve == 1">({{ _item.earnings
            }}%)</span>
          </div>
        ...
        <div class="li-r font-size-14 text-align-right flex flex-col gap-4px">
          ...
          <div class="label text-#0F172A font-size-18px font-700" v-if="item.status == 2">
            {{ t("Ended") }}
          </div>
    ...
    <div
      ...
      <div class="top-li gap-30 flex items-center">
        <div class="l flex flex-[2] flex-shrink-0 items-center gap-16 h-40">
          <div class="logo w-40 h-40 rounded-full overflow-hidden flex-shrink-0">
      ...
      <div class="discountDivider w-full h-1px bg-#EBECEF mt-12px mb-12px"></div>
      <div class="tag-box flex items-center justify-between gap-8px mb-20px">
        ...
        <div class="tag-item flex items-center gap-4px px-12px py-6px rounded-full bg-#FFF0F0">
          <div class="icon w-16px h-16px flex items-center justify-center">
            <svg width="14" height="14" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg">
              ...
              <path d="M5.25 8.75012L8.75 5.25012" stroke="#F14437" stroke-width="1.5" stroke-linecap="round" />
              <path
        ...
        <!-- 第三个标签：绿色主题 -->
        <div class="tag-item flex items-center gap-4px px-12px py-6px rounded-full" :class="item.income >= 0
          ...
          <div class="icon w-16px h-16px flex items-center justify-center">
            <svg width="17" height="16" viewBox="0 0 17 16" fill="none" xmlns="http://www.w3.org/2000/svg"
              ...
              <path
                ...
                :stroke="item.income >= 0 ? '#079455' : '#F14437'" stroke-linecap="round" stroke-linejoin="round" />
              <path d="M8.49998 5.3335L5.83331 8.00016M8.49998 5.3335V10.6668M11.1666 8.00016L8.49998 5.3335"
                :stroke="item.income >= 0 ? '#079455' : '#F14437'" stroke-linecap="round" stroke-linejoin="round" />
            </svg>
          </div>
          <span class="text-#32B464 font-size-14px font-500">{{ item.income }}%</span>
        </div>
      </div>
      ...
      <div class="li flex justify-between gap-24px">
        ...
        <div class="li-r">
          {{ _item.unit || "MX$" }}
          <span :class="item.income_price >= 0 ? ' color-#32B464!' : ' color-#DC3232!'
            ">{{ addCommasToNumber(_item.income_price) }}</span>
        </div>
      </div>
      <div class="li flex justify-end">
        <van-button type="primary" :color="[1, 3].includes(_item.status) ? '#6B39F4' : '#b5b5b5'" class="w-full"
          @click="handleClickSubmit">{{ t(orderStatusEnum[_item.status]) }}</van-button>
      </div>
    </div>
  </div>
</template>
...
const handleClickSubmit = () => {
  ...
  if (props.itemType == "order" && _item.value.status != 1) {
    if (_item.value.status == 3) {
      emits("handleClickBtn", { ..._item.value, type: props.itemType });
      return;
    }
