<template>
  <div>
    <div
      class="discont-item-content w-full overflow-hidden flex flex-col mb-29px border-bottom"
      v-if="props.itemType == 'discount'"
      @click="handleClickItem"
    >
      <div class="top-li flex items-center relative">
        <div
          class="mark absolute top-0 left-0 w-full h-full z-10"
          @click="handleClickItem"
        ></div>
        <miniChart
          :exchange="item.full_name"
          :symbol="item.tradingview_name"
          :chartId="item.id + ''"
        >
        </miniChart>
      </div>
      <div class="top-li flex items-center" v-if="false">
        <div class="l flex flex-[2] flex-shrink-0 items-center gap-6 h-40">
          <div class="logo w-40 h-40 rounded-full overflow-hidden">
            <img
              :src="item.tradingInfo.baseAssetInfo.logo"
              alt=""
              v-if="item.tradingInfo && item.tradingInfo.baseAssetInfo"
              class="w-full h-full"
            />
          </div>
          <div
            class="name flex flex-col justify-between font-size-14px color-#0F172A font-700 h-40"
          >
            <div class="n">
        ...
        <div class="r text-nowrap flex-shrink-0 justify-end text-align-right">
          <Kline
            :nameId="'myChart10' + item.tradingPairsId"
            :areaStyle="true"
          ...
          <div
            class="price"
            :class="{
              'color-#1DCE5C': _item.increase >= 0,
              'color-#E11D48': _item.increase < 0,
            }"
          >
            {{ _item.unit }} {{ _item.close || "-" }}
          </div>
        </div>
      </div>
      ...
      <div
        class="li pt-20px pb-24 li-x-border flex justify-between items-center px-12 li-border"
      >
        <div class="li-l font-size-18 flex flex-col gap-4px w-full">
          <!-- <div class="rate font-size-16 text-#64748B flex items-center justify-between ">
                        <div class="flex-1">
                            MX$ {{ addCommasToNumber(_item.price) }}
                        </div>

                    </div> -->
          <div class="font-size-16 text-#64748B">
            {{ _item.discount_rate }}%
          </div>
          <div class="price text-#0F172A font-700 text-18">
            MX$ {{ addCommasToNumber(_item.discount_price) }}
          </div>
        </div>
        <div class="li-r font-size-14 text-align-right flex flex-col gap-4px">
          <div class="label text-#64748B font-size-14px text-nowrap">
            {{ t("SellableTime") }}
          </div>
          <div
            class="label text-#0F172A font-size-18px font-700"
            v-if="item.status == 2"
          >
            {{ t("Ended") }}
          </div>
          <div class="label text-#0F172A font-size-18px font-700" v-else>
            {{ item.sale_end_time }}
          </div>
        </div>
      </div>
    ...
    >
      <div class="top-li flex items-center">
        <div class="l flex flex-[2] flex-shrink-0 items-center gap-16 h-40">
          ...
          <div
            class="name flex flex-col justify-center font-size-14px color-#0F172A font-700 h-40"
          >
            <div class="n">
              {{ item.full_name || "" }}
            </div>
      ...
      <div
        class="discountDivider w-full h-1px bg-#EBECEF mt-12px mb-12px"
      ></div>
      <div class="tag-box flex items-center justify-between gap-8px mb-20px">
        <!-- 第一个标签：红色主题 -->
        <div
          class="tag-item flex items-center gap-4px px-12px py-6px rounded-full bg-#FFF0F0"
        >
          ...
          <span class="text-#DC3232 font-size-14px font-500"
            >{{ item.discount_rate }}%</span
          >
        </div>
        ...
        <!-- 第二个标签：橙色主题 -->
        <div
          class="tag-item flex items-center gap-4px px-12px py-6px rounded-full bg-#FFF5E6"
        >
          <div class="icon w-16px h-16px flex items-center justify-center">
            <svg
              width="14"
          ...
          <span class="text-#FF8C00 font-size-14px font-500">{{
            item.sale_end_time
          }}</span>
        </div>
        ...
        <!-- 第三个标签：绿色主题 -->
        <div
          class="tag-item flex items-center gap-4px px-12px py-6px rounded-full"
          :class="
            item.income >= 0
              ? 'bg-#E6F9F0 color-#32B464'
              : 'bg-#FFF5F5 color-#DC3232'
          "
        >
          <div class="icon w-16px h-16px flex items-center justify-center">
            <svg
            ...
            >
              <path
                ...
                :stroke="item.income >= 0 ? '#079455' : '#F14437'"
                stroke-linecap="round"
          ...
          <span class="text-#32B464 font-size-14px font-500"
            >{{ item.income }}%</span
          >
        </div>
      </div>
      ...
      <div class="li flex justify-between gap-24px">
        <div class="li-l font-size-16 text-#717680">
          {{ _item.status != 2 ? t("Market price") : t("Sell price") }}
        </div>
        <div class="li-r">
          MX$
          {{
            _item.status != 2
              ...
              : addCommasToNumber(_item.sell_price)
          }}
        </div>
      </div>
      <div class="li flex justify-between gap-24px">
        <div class="li-l font-size-16 text-#717680">{{ t("Buy price") }}</div>
        <div class="li-r">
          {{ _item.unit || "MX$" }}
          {{ addCommasToNumber(_item.discount_price) }}
        </div>
      </div>

      <div class="li flex justify-between gap-24px">
        <div class="li-l font-size-16 text-align-right text-#717680">
          {{ t("Yield") }}
        </div>
      ...
      <div class="li flex justify-end">
        <van-button
          type="primary"
          :color="_item.status == 1 ? '#6B39F4' : '#b5b5b5'"
          class="w-full"
