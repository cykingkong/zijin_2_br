    ...
    <div
      ...
      <div class="top-li flex items-center" @click="handleClickDetail">
        <div class="l flex flex-shrink-0 items-center gap-6 justify-between ">
          <div class="name font-size-16 font-700 text-#0F172A">{{ item.name }} <svg width="14" height="14"
      ...
      <div class="fundDivider w-full h-1px bg-#EBECEF mt-10px mb-12px" @click="handleClickDetail"></div>

      <div class="li flex items-center justify-between text-12px text-#A3A7AE" @click="handleClickDetail">
        <div class="li-l ">
          <div class="l1">预约开始时间</div>
          <div class="t">
      ...
      <div class="li flex justify-between items-center mb-22px p-8 gap-8px bg-#F8F9FD rounded-12px mt-12px"
      ...
      <div class="li flex justify-end gap-12px">
        ...
        <van-button type="primary" :color="_item.status == 1 ? '#1989fa' : '#b5b5b5'" @click="handleClickSubmit"
          size="small" class="font-size-14! w-full h-35px rounded-35px!">{{ t(statusEnum[_item.status]) }}</van-button>
      </div>
    </div>
    <div
      class="order-item-content w-full px-10 py-12 font-size-12 flex flex-col gap-10px bg-#F8F9FD mb-16px rounded-12px"
      v-if="props.itemType == 'order'" @click.stop="handleClickOrderDetail">
      ...
      <div class="line w-full bg-#693CFF1F rounded-16px h-16px">
        <div class="line-inner bg-#693CFF rounded-16px h-16px" :style="{ width: item.calculateProgress + '%' }">
        </div>
      ...
      <div class="li flex justify-between items-center text-#717680">
        ...
        <div class="li-r font-size-14 text-align-right">
          <div class="rate">{{ t("dailyYield") }}:{{ _item.daily_yield }}%</div>
          <div class="rate">
            {{ t("totalYield") }}: <span :class="_item.total_yield >= 0 ? 'up' : 'down'"> {{ _item.total_yield }}
              %</span>
          </div>
      ...
      <div class="li flex gap-12px justify-end items-center">
        ...
        <van-button :color="_item.status == 1 ? '#1BC256' : '#b5b5b5'" @click="handleClickSubmit" size="small"
          class="font-size-14! flex-1 h-36px!" v-if="_item.status !== 7">{{ t(orderStatusEnum[_item.status])
          }}</van-button>
        <van-button type="primary" color="#1BC256" v-if="_item.status == 7" @click="handleClickOrder(1)" size="small"
          class="font-size-12! flex-1 h-36px!">{{ '续期' }}</van-button>
        <van-button type="primary" color="#E82C81" v-if="_item.status == 7" @click="handleClickOrder(2)" size="small"
          class="font-size-12! flex-1  h-36px!">{{ t("赎回") }}</van-button>
      </div>
    </div>
  </div>
