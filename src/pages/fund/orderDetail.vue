<template>
  <div class="wallet">
    <div class="h-165px w-full top-info relative">
      ...
      >
        <template #title> Order Detail </template>
      </VanNavBar>
      <div
      ...
      >
        ...
        <div class="line w-full bg-#693CFF1F rounded-16px h-16px">
          <div
            ...
            :style="{ width: info.calculateProgress + '%' }"
          ></div>
        </div>
        <div class="li flex justify-between items-center text-#717680">
          ...
          <div class="li-r font-size-14 text-align-right">
            <div class="rate">{{ t("D.Y") }}:{{ info.daily_yield }}%</div>
            <div class="rate">
              {{ t("T.Y") }}:
              <span :class="info.total_yield >= 0 ? 'up' : 'down'">
                {{ info.total_yield }} %</span
              >
            </div>
          </div>
        </div>
    ...
    <div class="list w-full px-14px mt-90px">
      ...
      <div
        ...
        :key="e"
      >
        <div class="label color-#64748B text-14px">{{ i.created_at }}</div>
        <div class="value flex justify-between h-40px items-center gap-12px">
          <div class="text-#0F172A text-14px font-bold color-#693CFF">
            + MX$ {{ i.amount }}
          </div>

          <svg
            class="w-20px h-20px"
            :class="radioList.includes(1) ? 'radio-active' : ''"
            viewBox="0 0 20 20"
            fill="none"
