...
<template>
  <div class="m-x-a w-full max-h-100vh">
    ...
    <div class="login-form p-24">
      ...
      <inputCom
        v-model:value="postData.account"
        :placeholder="type == '1' ? t('Phone') : t('Email')"
        :onlyRead="false"
        :type="'text'"
        v-if="type == '2'"
      ></inputCom>
      ...
      <div class="flex justify-between items-center mt-16px mb-24px gap-16">
        <div
          class="left flex font-size-14px font-500 flex-shrink-0 gap-12px flex-shrink-0"
        >
          <div
            class="radio w-16px h-16px rounded-4px border-1px"
            :class="remember ? 'radio-active' : ''"
            @click="remember = !remember"
          ></div>
          {{ t("Remember me") }}
        </div>
        <div
          class="right color-#6B39F4 font-size-14px font-700 cursor-pointer flex-shrink-0"
