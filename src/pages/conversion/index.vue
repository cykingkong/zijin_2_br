<template>
  <div class="conversion-page p-12">
    ...
    <inputCom :label="t('price')" class="mb-12" :placeholder="t('input.PleaseEnter')" v-model:value="form.amount"
      :tips="tips">
      <template #sendCode>
...
watch(
  ...
  (val) => {
    ...
    if (val == 0) {
      ...
      tips.value = `${t("Available balance")}:${conversionInfoData.value.baseAssetSymbolName
        } ${addCommasToNumber(Number(price.value))}`;
    } else {
      tips.value = `${t("Available balance")}:${conversionInfoData.value.quoteAssetSymbolName
...
watch(
  ...
  (val) => {
    if (balanceType.value == 0) {
      ...
      canhasAmountTips.value = `${t("Estimated to obtain")}:${conversionInfoData.value.quoteAssetSymbolName
        } ${addCommasToNumber(canhasAmount.value)}`;
    } else {
      canhasAmount.value = form.amount
        ? Number(form.amount) * conversionInfoData.value.quoteAssetAmountRate
        : 0;
      canhasAmountTips.value = `${t("Estimated to obtain")}:${conversionInfoData.value.baseAssetSymbolName
        } ${addCommasToNumber(canhasAmount.value)}`;
    }
  }
);
