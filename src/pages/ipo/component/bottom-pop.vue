<template>
  <van-popup v-model:show="showPicker" destroy-on-close round :position="'bottom'" :safe-area-inset-bottom="true" z-index="10000">
    <div class="w-full p-12 flex flex-col gap-12">
      ...
      <inputCom :label="popType == 'ipo' ? t('Subscription number') : t('Quantity sold')
        " :placeholder="t('input.PleaseEnter')" v-model:value="form.number" v-if="popType == 'ipo'">
      </inputCom>
      ...
      <div class="w-full flex gap-12">
        <div class="btn-box flex-1">
          <van-button type="default" class="h-40!" plain block @click="showPicker = false" v-if="popType == 'order'">{{
            t("cancel") }}</van-button>
          <van-button type="default" class="h-40!" plain block @click="confirm" v-else>{{ t("subscription")
            }}</van-button>
        </div>
        <div class="btn-box flex-1">
          <van-button type="primary" class="h-40!" block @click="confirm" v-if="popType == 'order'">{{ t("confirm")
            }}</van-button>
          <van-button type="primary" class="h-40!" block @click="confirmAll" v-else>{{ t("subscription all")
            }}</van-button>
        </div>
      </div>
    </div>
