  ...
  <div class="buy-success-content bg-white min-h-screen flex flex-col">
    ...
    <div class="flex-1 flex flex-col items-center pt-40px px-24px">
      ...
      <div class="success-icon-section mb-32px relative">
        <!-- 主成功图标 -->
        <img
          src="@/assets/image/orderSuccess.png"
      ...
      <div class="confirmation-info text-center mb-32px">
        <h1 class="title text-24px font-400 color-#0F172A mb-8px">
          {{ t("Buy order received!") }}
        </h1>
        <div class="amount text-32px font-400 color-#0F172A mb-4px">
          MX$ {{ info.total_amount || info.amount }}
        </div>
      </div>

      <!-- 支付详情卡片 -->
      <div
        class="payment-details-card bg-white rounded-16px px-24px pb-24px w-full shadow-lg"
      >
        <div class="card-header text-center mb-24px">
          <h2 class="text-18px font-600 color-#374151">
            {{ t("Payment Details") }}
          </h2>
        </div>
        ...
        <div class="details-list space-y-16px" v-if="buyType == 'discount'">
          ...
          <div class="detail-row flex justify-between items-center">
            <div class="label text-14px color-#6B7280">{{ t("Quantity") }}</div>
            <div class="value text-14px font-600 color-#0F172A">
              {{ info.quantity }} {{ t("Quantity") }}
            </div>
          </div>

          <!-- 购买金额 -->
          <div class="detail-row flex justify-between items-start">
            <div class="label text-14px color-#6B7280">
              {{ t("Purchase amount") }}
            </div>
            <div class="value text-right">
              <div class="text-14px font-600 color-#0F172A">
                MX$ {{ info.total_amount }}
              </div>
            </div>
          </div>
        ...
        <div class="details-list space-y-16px" v-if="buyType == 'stock'">
          ...
          <!-- 买入价格 -->
          <div class="detail-row flex justify-between items-start">
            <div class="label text-14px color-#6B7280">{{ t("Fee") }}</div>
            <div class="value text-right">
              <div class="text-14px font-600 color-#0F172A">
                MX$ {{ info.fee }}
              </div>
