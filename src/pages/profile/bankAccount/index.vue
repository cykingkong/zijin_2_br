  ...
  <div class="bankAccount p-24 pb-120">
    ...
    >
      <div class="content py-12px">
        <div
          class="mid w-40px h-6px bg-#E2E8F0 rounded-6px mx-auto mb-12px"
        ></div>
        <div
          class="title h-38px flex items-start justify-between border-b-1px border-b-#E2E8F0 border-b-solid mb16px"
        >
          <div class="left pl-24px text-16px font-700">{{ t("Picker") }}</div>
        </div>
        <div class="radio-group px-24px">
          <div
            class="radio-item w-full px-16px rounded-12px border-1px border-#E2E8F0 border-solid h-58px flex items-center justify-between mb-16px"
            :class="{ 'border-#6B39F4 bg-#F8F5FF': activeRadio == index }"
            v-for="(item, index) in radioList"
            :key="index"
            @click="activeRadio = index"
          >
            {{ item.title }}
            <div
