    ...
    <div class="wallet">
        <div class="h-265px w-full top-info">
            ...
            <div class="info-box px-24px flex flex-col gap-4px h-102px">
                <div class="t1 text-#B59CFA text-16px">Balance</div>
                <div class="t1 text-#fff text-34px line-height-140%">MX$ {{ addCommasToNumber(userInfo.user_balance) }}
                </div>

            </div>
            <div class="btn-box w-full flex gap-10px px-24px mt-33px">
                <div class="rounded-12px border-#fff border-1px border-solid h-36px flex-1 flex items-center gap-12px justify-center text-#fff text-16px"
        ...
        <div class="list w-full px-14px">
            <div class="list-title  flex  items-center h-40px justify-between">
                <div class="text-#0F172A text-16px font-700">Transactions</div>
                <div class="text-#6B39F4 text-16px" @click="reload">Reload</div>
            </div>
            <div class="list-content">
                <div class="list-items mb-24" v-for="(item, index) in list" :key="index">
                    ...
                    <item class="mt-16px" v-for="(el, k) in item.transactions" :key="k" :item="el">
                        <template #left>
                            <div class="left h-46px flex items-center gap-16px">
                                <div
                                    ...
                                    <!-- in-Svg -->
                                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" :class="el.type"
                                        v-if="el.type == 'income'" xmlns="http://www.w3.org/2000/svg">
                                        <path
                                    ...
                                    <!-- out-Svg -->
                                    <svg width="22" height="22" viewBox="0 0 22 22" fill="none" :class="el.type"
                                        v-if="el.type == 'expend'" xmlns="http://www.w3.org/2000/svg">
                                        <path
                                ...
                                <div class="info h-46px flex flex-col justify-between ">
                                    <div class="name text-#0F172A text-14px font-bold">{{ el.log_type }}</div>
                                    <div class="time text-#64748B text-12px ">{{ el.time }}</div>
                                </div>
                            </div>
                        </template>
                        <template #right>
                            <div class="right h-46px flex items-center gap-16px">
                                MX$ {{ el.amount }}
                            </div>
                        </template>
                    </item>
