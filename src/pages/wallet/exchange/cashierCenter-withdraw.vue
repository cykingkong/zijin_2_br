<template>
  <div class="cashier-center-content px-12 w-full">
    ...
    <div class="info mt-32" v-if="info">
      ...
      <div
        class="min-count-fee text-#64748B font-size-14px mx-a text-center mt-6px font-700 overflow-y-auto"
      >
        {{ t("Withdraw fee") }} MX${{ fee }}
      </div>
      <div class="mt-30px">
        <item class="mt-62px">
          <template #left>
        ...
        <div class="tips mt-45px">
          ...
          <div
            class="l flex items-start color-#64748B text-14px gap-8px pl-6px"
          >
            <div class="dot w-4 h-4 rounded-full bg-#64748B"></div>
            {{ t("Withdrawal time on business days is") }}
            {{ withdrwaInfo.withdraw_hour_before }} AM ~
            {{ withdrwaInfo.withdraw_hour_after }} PM
          </div>
          <div
            class="l flex items-start color-#64748B text-14px gap-8px pl-6px"
          >
            <div class="dot w-4 h-4 rounded-full bg-#64748B"></div>
            {{ t("Minimum withdrawal amount") }} MX$
            {{ withdrwaInfo.withdraw_amount_before }} ~ MX$
...
const fee = computed(() => {
  return addCommasToNumber(count.value * withdrwaInfo.value.bank_fee * 0.01);
});
