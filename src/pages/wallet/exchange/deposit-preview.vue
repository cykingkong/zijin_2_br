<template>
  <div class="deposit-preview bg-[#0F172A] min-h-screen">
    <!-- 头部导航 -->
    <VanNavBar :title="t('Deposit preview')" :fixed="true" clickable :left-arrow="true" @click-left="onBack"
      z-index="999" />
    ...
    <div class="content px-12 pt-[80px] pb-[24px]">
      ...
      <div class="main-card bg-white rounded-[16px] p-[24px] mb-[24px]">
        ...
        <div class="top-section text-center mb-[32px]">
          <div class="icon-wrapper w-[80px] h-[80px] rounded-full mx-auto mb-[16px] flex items-center justify-center">
      ...
      <div class="note-section bg-white rounded-[16px] p-[24px] mb-[24px]">
        <div class="note-title text-[16px] font-bold color-[#DC2626] mb-[16px]">
          {{ t("Notice") }}
        </div>
        <div class="note-content">
          ...
          <div class="note-item flex items-start gap-[8px]">
            ...
            <div class="text text-[14px] color-[#0F172A] leading-[20px]">
              {{
                t(
                  `This asset currently does not support smart contract deposits! Do not use smart contract deposits,
              otherwise assets will not arrive and cannot be recovered.`
                )
              }}
            </div>
          </div>
    ...
    <!-- 底部按钮 -->
    <BottomButton bg-color="#0F172A" :button-text="`${t('Copy wallet address for payment')}`"
