<template>
    <div class="deposit-preview bg-#0F172A min-h-screen">
        <!-- 头部导航 -->
        <VanNavBar title="充值预览" :fixed="true" clickable :left-arrow="true" @click-left="onBack" z-index="999" />
        ...
        <div class="content px-12 pt-80px pb-24px">
            ...
            <div class="main-card bg-white rounded-16px p-24px mb-24px">
                ...
                <div class="top-section text-center mb-32px">
                    <div
                        ...
                        <img :src="depositData.logo" alt="" class="w-full h-full " v-if="depositData.logo">

                    </div>
                    <div class="deposit-type text-14px color-#6B7280 mb-8px">Deposit ({{ depositData.unit }})</div>
                ...
                <div class="details-section">
                    ...
                    <div class="detail-row flex justify-between items-start py-16px border-b  border-#E5E7EB">
                        <div class="label text-14px color-#0F172A">钱包地址</div>
                    ...
                    <div class="detail-row flex justify-between items-center py-16px border-b border-#E5E7EB">
                        <div class="label text-14px color-#0F172A">{{ depositData.unit }} 数量</div>
                        <div class="value text-14px color-#0F172A">{{ depositData.rate }} {{ depositData.unit }}</div>
                    </div>

                    <!-- 充值到账 -->
                    <div class="detail-row flex justify-between items-center py-16px">
                        <div class="label text-14px color-#0F172A">充值到账</div>
            ...
            <div class="note-section bg-white rounded-16px p-24px mb-24px">
                <div class="note-title text-16px font-700 color-#DC2626 mb-16px">注意</div>
                <div class="note-content">
                    <div class="note-item flex items-start gap-8px mb-12px">
                        <div class="dot w-6px h-6px rounded-full bg-#DC2626 mt-8px flex-shrink-0"></div>
                        <div class="text text-14px color-#0F172A leading-20px">
                            请勿向当前地址进行任何非{{ depositData.unit }}的充值,否则资产将不会到账且无法找回。
                        </div>
                    </div>
                    <div class="note-item flex items-start gap-8px mb-12px">
                        <div class="dot w-6px h-6px rounded-full bg-#DC2626 mt-8px flex-shrink-0"></div>
                        <div class="text text-14px color-#0F172A leading-20px">
                            请务必提供您的正确钱包地址,以确保充值成功。
                        </div>
                    </div>
                    <div class="note-item flex items-start gap-8px">
                        <div class="dot w-6px h-6px rounded-full bg-#DC2626 mt-8px flex-shrink-0"></div>
                        <div class="text text-14px color-#0F172A leading-20px">
                            该资产当前暂不支持智能合约充值!请勿使用智能合约充值,否则资产将不会到账且无法找回。
                        </div>
                    </div>
                </div>
            </div>
        ...
        <!-- 底部按钮 -->
        <BottomButton bg-color="#0F172A" button-text="复制钱包地址进行支付" @click="handleCopyAddress" />
    </div>
</template>
