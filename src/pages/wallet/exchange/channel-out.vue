<template>
  <div class="exchange-in-content py-16 px-24 pb-95px">
    ...
    <div class="radio-group flex flex-col gap-16px mb-16px" v-for="(item, index) in payTypeList" :key="index">
      ...
      <item v-for="(listItem, key) in item.list" :key="key" @click="payType = listItem.id">
        <template #left>
          <div class="left h-46px flex items-center gap-16px">
            ...
            <div class="info h-46px flex flex-col justify-between ">
              <div class="name text-#0F172A text-14px font-bold">{{ listItem.name }}</div>
              <div class="name2 text-#64748B text-12px">金额范围: MX${{ listItem.min_amount }} ~ MX${{ listItem.max_amount
                }}</div>
            </div>
          </div>
        </template>
...
const handleBuyClick = () => {
  let item = payTypeList.value.find(item => item.list.find(listItem => listItem.id == payType.value))
