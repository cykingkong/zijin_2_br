...
const handleClickIndicator = (val) => {
  ...
  router.push("/quotes/detail?symbol=" + val.symbol);
};
const clickNotice = () => {
  ...
  <div class="quotes">
    ...
    <van-tabs v-model:active="activeName" shrink @change="handleClickTabs">
      <div class="list-content flex flex-col gap-16px px-12">
        ...
        <!-- 搜索结果列表 -->
        <div
          v-else-if="!searchLoading && marketData.length > 0"
          class="list-items relative"
          v-for="(item, index) in marketData"
          :key="index"
        >
          <div
            class="mark absolute left-0 top-0 w-full h-full"
            @click="handleClickIndicator(item)"
          ></div>
          <singleTicker
        ...
        <!-- 无搜索结果 -->
        <div
          v-else-if="!searchLoading && marketData.length === 0 && search"
