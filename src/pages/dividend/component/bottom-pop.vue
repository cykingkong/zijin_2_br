<template>
  <van-popup v-model:show="showPicker" destroy-on-close round :position="'bottom'" :safe-area-inset-bottom="true">
    <div class="w-full p-12 flex flex-col gap-12">
      ...
      <inputCom :label="popType == 'discount' ? t('BuyShare') : t('SellShare')" :placeholder="t('input.PleaseEnter')
        " v-model:value="form.number" :tips="tips" v-if="popType == 'discount'">
      </inputCom>
      <div class="sell-tips font-size-18 font-500" v-if="popType == 'order'">
        {{ t('Dividend stocks will be sold, please confirm!') }}
      </div>
