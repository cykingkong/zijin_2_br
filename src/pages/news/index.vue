<template>
    <div class="community p-16 flex flex-col">
        <div class="flex w-full text-center pb-20 border-b-solid border-b-[1px] border-b-[#0000000D] mb-20">
            <div class="flex-1">
                <div class="value font-bold mb-4">88</div>
                <div class="label font-bold mb-4">{{ t('Total Submit') }}</div>
            </div>
            <div class="flex-1">
                <div class="value font-bold mb-4">88</div>
        ...
        <div class="tag-li flex flex-nowrap gap-4">
            <div class="tag-item font-bold px-10 py-6.5px text-nowrap text-[12px] rounded-[8px] border border-solid"
                :class="activeIndex == index ? 'bg-[#CED0D8] color-[#161616] border-[#CED0D8] border-solid border border-solid' : 'border-[#0000001A] bg-[#0000000D] color-[#00000033]'"
        ...
        <div class="item w-full p-16 rounded-16" v-for="(item, index) in logList" :key="index"
            ...
            <div class="picture-box">
                ...
                <div class="w-full min-h-[120px] rounded-[16px] overflow-hidden bg-[#f5f5f5] relative"
                    ...
                    <!-- 蒙版层 (图片状态) -->
                    <div class="absolute inset-0 bg-black/20 pointer-events-none flex items-end p-8 z-10">
                        <svg v-if="statusIconMap[imgItem.status]" class="w-24 h-24" viewBox="0 0 20 20" fill="none"
                            xmlns="http://www.w3.org/2000/svg">
                            <path :d="statusIconMap[imgItem.status].path" :fill="statusIconMap[imgItem.status].color" />
                        </svg>
                    </div>
                </div>
                ...
                <div class="w-full grid grid-cols-3 gap-8" v-if="[2, 3].includes(item.images.length)">
                    <div class="w-full h-120 rounded-[16px] overflow-hidden bg-[#f5f5f5] flex-1 relative"
                        ...
                        <!-- 蒙版层 (图片状态) -->
                        <div class="absolute inset-0 bg-black/20 pointer-events-none flex items-end p-6 z-10">
                            <svg v-if="statusIconMap[imgItem.status]" class="w-24 h-24" viewBox="0 0 20 20" fill="none"
                                xmlns="http://www.w3.org/2000/svg">
                                <path :d="statusIconMap[imgItem.status].path"
                                    :fill="statusIconMap[imgItem.status].color" />
                            </svg>
                        </div>
                    </div>
                </div>
