<template>
  <div class="tradingview-widget-container">
    ...
    ></div>
    <div
      :id="widgetId + '_company'"
      :key="widgetKey + '_company'"
      class="tradingview-widget-container__widget_company"
    ></div>
  </div>
</template>
...
const widgetIdCompany = computed(
  () => `tradingview_widget_${widgetKey.value}_company`
);
const { t } = useI18n();
// 创建TradingView图表
...
watch(
  () => props.baseAssetInfo.symbol,
  () => {
    if (props.baseAssetInfo.symbol) {
      createWidget();
      createWidgetCompany();
    }
  }
